        ...
        match event {
            RedrawRequested(..) => {
                ...
                egui::CentralPanel::default().show(&platform.context(), |ui| {
                    // Create a window.
                    egui::Window::new("Exponential").show(ui.ctx(), |ui| {
                        ui.label(format!("Money: {}", exponential.money.to_fixed(0)));
                        if ui.button("Add 1").clicked() {
                            exponential.money += Decimal::new(1.0);
                        }
                        if ui.button("Double").clicked() {
                            exponential.money = exponential.money.pow(&Decimal::new(2.4));
                        }
                    });
                });
